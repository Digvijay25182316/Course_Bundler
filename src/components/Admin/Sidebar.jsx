...
function Sidebar() {
  ...
  return (
    <VStack spacing={'8'} p={'16'} boxShadow={'-2px 0 10px rgba(107,70,193,0.5)'}>
      ...
      <LinkButton Icon={RiAddCircleFill} text={'Course'} url={'createcourse'} active={location.pathname==='/admin/createcourse'}/>
...
return(
  <Link to={`/admin/${url}`}>
      <Button fontSize={"larger"} variant={'ghost'} colorScheme={active?'purple':""}><Icon style={{margin:"4px"}}/>{text}</Button>
  </Link>
)
}