...
function CourseModal({isOpen,onClose,id,deleteButtonHandler,courseTitle,lectures=[], addLectureHandler}) {
  ...
  return (
    ...
    <VideoCard title="React intro" description="This is a intro lecture where you will know the basics of react" num={1} lectureId={"6876657sdjfhksdjf"} courseId={id} deleteButtonHandler={deleteButtonHandler}/>
    </Box>
    <Box>
        <form onSubmit={e=>addLectureHandler(e,id,title,description,video)}>
            <VStack spacing={'4'}>
                ...
                <Input focusBorderColor='purple.300' placeholder='Title' value={title} onChange={(e)=>setTitle(e.target.video)}/>
                <Input focusBorderColor='purple.300' placeholder='Description' value={description} onChange={(e)=>setDescription(e.target.value)}/>
                <Input accept={"video/mp4"} required onChange={changeVideoHandler} type='file' focusBorderColor='purple.300' css={{...fileuploadStyle}}/>
                {
                   videoPrev&&(
...
function VideoCard({num,title,description,courseId,lectureId,deleteButtonHandler}){
    return(
        <>
